...
def get_chars(car_plate, char_w, char_h):
    ...
    cv2.imwrite('./carIdentityData/opencv_output/prebinImg.jpg',car_plate)
    cv2.imwrite('./carIdentityData/opencv_output/binImg.jpg', plates)
    ...
    for i,addr in enumerate(char_addr_list):
        ...
        cv2.imwrite('./carIdentityData/opencv_output/pchar_{}.jpg'.format(i), char_img)
    return char_imgs
...
def pred_process(orig_img, colour):
    ...
    if (colour=="blue"):
        hsv_img = (h > 100) & (h < 125) & (s > 90) & (v > 90)
    if (colour=="yellow"):
        hsv_img = (h > 14) & (h < 24) & (s > 90) & (v > 90)
    hsv_img = hsv_img.astype('float32')
    hsv_img = cv2.blur(hsv_img, (3, 3))
    mix_img = np.multiply(sobel_img, hsv_img)
    ...
    cv2.imwrite('./carIdentityData/opencv_output/preImg.jpg', close_img)
...
def verify_color(rotate_rect, src_image, colour):
    img_h, img_w = src_image.shape[:2]
    mask = np.zeros(shape=[img_h+2, img_w+2], dtype=np.uint8)

    connectivity = 4 #种子点上下左右8邻域与种子颜色值在[loDiff,upDiff]的被涂成new_value，也可设置8邻域
    loDiff,upDiff = 30,30
    new_value = 255
    flags = connectivity
    ...
    for i in range(rand_seed_num):
        ...
        #限制随机种子必须是车牌背景色
        if (colour=="blue" and ((h[row,col]>100) & (h[row,col]<125) & (s[row,col]>110) & (v[row,col]>110))):
            cv2.floodFill(src_image, mask, (int(col),int(row)), (255, 255, 255), (loDiff,) * 3, (upDiff,) * 3, flags)
            seed_cnt += 1
            if seed_cnt >= valid_seed_num:
                break
        if (colour=="yellow" and ((h[row,col]>14) & (h[row,col]<24) & (s[row,col]>100) & (v[row,col]>100))):
            cv2.floodFill(src_image, mask, (int(col),int(row)), (255, 255, 255), (loDiff,) * 3, (upDiff,) * 3, flags)
            seed_cnt += 1
            if seed_cnt >= valid_seed_num:
                break
...
def locate_carPlate(orig_img, pred_image, colour):
    carPlate_list = []
    ...
    for i, contour in enumerate(contours):
        ...
        if verify_scale(min_rect):
            ...
            #漫水法进一步确定车牌
            #ret,min_rect = verify_color(min_rect, orig_img, colour)
            #if ret == False:
            #    continue
            ...
            car_plate = cv2.resize(car_plate, (136, 36))
            carPlate_list.append(car_plate)

    #cv2.imshow('contour', contour_img)
    cv2.imwrite('./carIdentityData/opencv_output/susImg.jpg', contour_img)
    #cv2.imshow('precontour', precontour_img)

    return carPlate_list

def plate_recongnition(img_path):
    cur_dir = sys.path[0]
    char_w, char_h = 20,20
    plate_model_path = os.path.join(cur_dir, './carIdentityData/model/plate_recongnize/model.ckpt-510.meta')
    char_model_path = os.path.join(cur_dir,'./carIdentityData/model/char_recongnize/model.ckpt-610.meta')
    img = cv2.imread(img_path)

    pred_img = pred_process(img, "blue")
    car_plate_list = locate_carPlate(img, pred_img, "blue")
    ret, car_plate = cnn_select_carPlate(car_plate_list, plate_model_path)
    if ret == False:
        pred_img = pred_process(img, "yellow")
        car_plate_list = locate_carPlate(img, pred_img, "yellow")
        ret, car_plate = cnn_select_carPlate(car_plate_list, plate_model_path)
        if ret == False:
            print("未检测到车牌")
            sys.exit()
    ...
    cv2.imwrite('./carIdentityData/opencv_output/recongnitionImg.jpg', car_plate)
    ...
    for i, char in enumerate(text):
        if not char:
            continue
        path = './chars/'+char+'.png'
        char_img = cv2.imdecode(np.fromfile(path, dtype=np.uint8), 1)
        cv2.imwrite('./carIdentityData/opencv_output/char_{}.jpg'.format(i), char_img)
...
if __name__ == '__main__':
    sys.exit(plate_recongnition('./1.png'))
