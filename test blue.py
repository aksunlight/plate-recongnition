...
def verify_color(rotate_rect,src_image):
    ...
    for i in range(rand_seed_num):
        ...
        #限制随机种子必须是车牌背景色
        if ((h[row,col]>100) & (h[row,col]<125)) & (s[row,col]>110) & (v[row,col]>110):
...
def locate_carPlate(orig_img, pred_image, car_plate_w, car_plate_h):
    ...
    for i,contour in enumerate(contours):
        #画出所有的轮廓
        #cv2.drawContours(allcontour_img, contours, i, (0, 255, 255), 2)
...
blue_img = (h > 100) & (h < 125) & (s > 90) & (v > 90)
